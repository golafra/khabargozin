...
- **حالت انتشار** (`PUBLISH_MODE`) — آیا واقعاً به تلگرام می‌رود یا فقط dry_run
- **پیام‌ها / خوشه‌نشده** — اگر خوشه‌نشده زیاد است، cluster task عقب است
- **منتشر امروز** — تعداد انتشار موفق
- **AI ناموفق** — نیاز به بررسی (`retry_ai_failed`)
- **هزینه AI** — نزدیک بودجه ماهانه؟
- **منابع stale** — چند ساعت fetch نشده‌اند
- **وضعیت خوشه‌ها / outbox** — توزیع status
- **فعالیت ۳ ساعت اخیر** — fetch / خوشه‌شده / چندمنبعی / زیر آستانه
- **رویدادهای چندمنبعی** — ادغام واقعی بین کانال‌ها

لینک سریع: «مسیر پردازش ۳ ساعت اخیر»

### ادغام بین کانال‌ها — چطور کار می‌کند؟

وقتی پیام جدید از **یک** کانال می‌آید، با **نزدیک‌ترین خوشه‌های باز** (از همه منابع) مقایسه می‌شود — pgvector، نه اسکن خطی همه پیام‌ها. پنجره merge: **`CLUSTER_ACTIVE_WINDOW_MINUTES`** (پیش‌فرض ۶ ساعت). آستانه شباهت: **۰.۷۲**.

اکثر پیام‌ها ادغام نمی‌شوند (تک‌منبعی → `below_threshold`). داشبورد عمداً **سیگنال** نشان می‌دهد، نه هزاران پیام (بخش «ادغام بین کانال‌ها» پایین‌تر).

### ۲. مسیر پردازش (`/pipeline`) — **مهم‌ترین صفحه**
