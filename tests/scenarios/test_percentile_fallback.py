"""Percentile fallback threshold."""

from datetime import datetime, timedelta, timezone
from unittest.mock import MagicMock, patch

from app.clustering.threshold import effective_threshold
from app.config import Settings


def test_percentile_fallback_when_few_clusters():
    settings = Settings(
    ...
    with patch("app.clustering.threshold.get_settings", return_value=settings):
        with patch("app.clustering.threshold.should_skip_clustering", return_value=(False, "steady")):
            assert effective_threshold(session) == 6.0
